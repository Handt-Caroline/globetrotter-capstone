...
#
# Managing (login required; shared catalogue - any signed-in user may touch
# any place, the same rule the global chat uses):
#   POST   /destinations             -> api.createDestination(payload)
#   PUT    /destinations/{id}        -> api.updateDestination(id, payload)
#   DELETE /destinations/{id}        -> api.deleteDestination(id)
#   POST   /destinations/media       -> api.uploadMedia(file)
#
...
def list_destinations(
    ...
    return [localise(d, lang) for d in results]


@router.post("", status_code=status.HTTP_201_CREATED)
def create_destination(payload: DestinationBody, current_user: dict = Depends(get_current_user)):
...
def delete_destination(destination_id: str, current_user: dict = Depends(get_current_user)):
    db = read_db()
    existing = _find(db, destination_id)
    if not existing:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Destination not found")
...
def get_destination(
    ...
    if not destination:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Destination not found")
    return localise(destination, lang)
